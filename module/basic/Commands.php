...
class Commands extends Module
{
    ...
    protected function trigger(array $arguments = [])
    {
        ...
        if ($mode != self::TRIGGER_DELETE) {
            ...
            $parsedArguments['channels'] = isset($parsedArguments['channels']) ? explode(',',
                $parsedArguments['channels']) : null;
            $parsedArguments['reply'] = isset($parsedArguments['reply']) ? str_replace('_', ' ',
                $parsedArguments['reply']) : null;
            $parsedArguments['notice'] = isset($parsedArguments['notice']) ? str_replace('_', ' ',
                $parsedArguments['notice']) : null;
        ...
        switch ($mode) {
            case self::TRIGGER_ADD:
                if (empty($parsedArguments['trigger'])) {
                    $this->reply('You shall type a trigger.');
                }
                if (empty($parsedArguments['reply']) && empty($parsedArguments['notice'])) {
                    $this->reply('You shall type reply message.');
                }
            ...
            case self::TRIGGER_DELETE:
                if (isset($arguments[1])) {
                    ...
                    if ($this->unsetCommand($name)) {
                        $this->reply("Command {$name} was deleted.");
                    } else {
                        $this->reply("Command {$name} not found.");
                    }
                } else {
                    $this->reply("Must type a name of command.");
                }
            case self::TRIGGER_EDIT:
                if (empty($parsedArguments['trigger'])) {
                    $this->reply('You shall type a trigger.');
                }
                $this->reply('Option not avaible.');
            default:
