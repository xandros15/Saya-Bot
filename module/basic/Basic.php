...
class Basic extends Module
{
    ...
    {
        if (strpos($this->bot->getMessage(),
                'This nickname is registered') !== false && !empty(Config::$personal->identify)
        ) {
            $this->message(Config::$personal->identify, null, IRC::IDENTIFY);
        }

        if ($this->ghost == true && strpos($this->bot->getMessage(),
                'Ghost with your nick has been killed') !== false
        ) {
